...
<div class="container">
    ...
    <div class="flex justify-center items-center my-12">
          ...
          <span></span>
					...
					echo "<p class='text-lg'>You are logged in, ".$_SESSION['username']."!</p>";
				}
				?>
...
if (isset($_SESSION['username'])) {
    ...
    $addTaskButton = '<a
        href="../../backend/taskProgress/addTask.php"
...
?>
				...
				<table class="w-full border-collapse border-slate-500 my-4">
					<thead>
						<tr>
							...
							<th class="border-y-2 px-4 py-2 text-left w-1/12">Delete?</th>
						...
						<tr class="<?= $rowcolor; ?>">
							...
							<td class="border-y-2 px-4 py-2">
								<input
									...
									class="form-checkbox h-5 w-5 text-blue-600" <?php if(!isset($_SESSION['username'])){ echo "disabled"; }?>	 />
							</td>
							<td class="border-y-2 px-4 py-2">
							<?php
								if(isset($_SESSION['username'])){
							?>
									...
									<select class="block w-1/2 p-1 border border-gray-400 rounded" onchange="updateProgress(<?=$row['id']?>, this.value)">
										...
										<option value="2" <?php if($row["progress"] == 2){ echo "selected"; } ?>>Belum Mulai</option>
										<option value="3" <?php if($row["progress"] == 3){ echo "selected"; } ?>>Selesai</option>
									</select>
		...
		<script>
			function updateDone(taskId, isChecked) {
				...
				xhr.open("POST", "../../backend/taskProgress/deleteTask.php", true);
				xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");

				let data = "taskId=" + taskId + "&isChecked=" + isChecked;
				xhr.send(data);
			}

			function updateProgress(taskId, selectedProgress) {
				let xhr = new XMLHttpRequest();
				xhr.open("POST", "../../backend/taskProgress/updateProgress.php", true);
				xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
