...
if (isset($_POST['taskId'])) {
    ...
    $progress_id = filter_var($_POST['selectedProgress'], FILTER_VALIDATE_INT);
    if ($task_id !== false) {
        // Prepare a SQL statement to delete the task
        $sql = "UPDATE tasks SET progress = :progress WHERE id = :id";
        $stmt = $kunci->prepare($sql);
        
        // Bind the task ID to the statement
        $stmt->bindParam(':progress', $progress_id, PDO::PARAM_INT);
